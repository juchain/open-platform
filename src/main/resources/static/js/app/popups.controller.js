...
(function() {
    ...
    function ImportAddressCtrl($scope, $stomp, $element, close) {
        ...
        $scope.onImportAddressConfirmed = function() {
            console.log('onImportAddressConfirmed1');
            ...
            // $stomp.send('http://blockchain.blockshine.net/account/register', $scope.importAddressData);
    ...
    function NewAddressCtrl($scope, $stomp, $element, close) {
        ...
        $scope.onNewAddressConfirmed = function() {
            console.log('onImportAddressConfirmed2');
            ...
            $.ajax({
                type:"POST",
                data:JSON.stringify($scope.newAddressData),
                dataType:'json',
                contentType: "application/json",
                url:'http://blockchain.blockshine.net/account/register',
                success : function(data) {
                    if(data&&data.code===0){
                        console.log(data);
                        localStorage.setItem("coinname",$scope.newAddressData.address);
                        window.location.href="/checking.html";
                    }else {
                        showErrorToastr('FORM VALIDATION', data.message);
                    }
