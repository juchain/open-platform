...
contract DSTContract is StandardToken{
    ...
    function DSTContract(){
    
      executive   = msg.sender;  
      name        = "testName";
      symbol      = "SMB";

      hackerGold = HackerGold(0x0000000000000000000000000000000000000000);
      eventInfo  = EventInfo(0x0000000000000000000000000000000000000000);
    }
     ...
     */
    function setEtherPrice(uint qtyForOneEther)
                                                onlyExecutive {
         etherPrice = qtyForOneEther;
     ...
     *                         issuence
     */
    function toggleTokenIssuance() returns (bool enabled) {
        return ableToIssueTokens = !ableToIssueTokens;
    }
     ...
     */
    function submitEtherProposal(uint requestValue, string url)
     ...
     */
    function submitHKGProposal(uint requestValue, string url)
    ...
    function proposalsSize() returns (uint size) {
     ...
     *
     */
    function redeemProposalFunds(uint i) returns (bool redeemed) {

        bytes32 id = listProposals[i].id;
